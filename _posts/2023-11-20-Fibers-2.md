...
Since we're playing around with only two threads, the scheduler would likely initiate a second platform thread to complete the task attached to the virt-1. For the sake of the example and to force the test environment to behave deterministic, I explicitly set the parallelization parameter of the VM to one thread before I started the application: 
